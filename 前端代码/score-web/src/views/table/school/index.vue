...
import { addSchoolDataApi, delSchoolDataApi, delBatchesDataApi, updateSchoolDataApi, getSchoolPageDataApi, exportFileApi } from "@/api/school"
...
import { Refresh, CirclePlus, Delete, RefreshRight, Upload, Download, Search } from "@element-plus/icons-vue"
...
//#region 增
const DEFAULT_FORM_DATA: SchoolData = {
  id: undefined,
  schoolId: 0,
  schoolName: "",
}
...
const formData = ref<SchoolData>(cloneDeep(DEFAULT_FORM_DATA))
const formRules: FormRules<SchoolData> = {
  schoolName: [{ required: true, trigger: "blur", message: "请输入学院名称" }]
}
const handleCreateOrUpdate = () => {
  formRef.value?.validate((valid: boolean, fields) => {
    if (!valid) return console.error("表单校验不通过", fields)
    loading.value = true
    const api = formData.value.id === undefined ? addSchoolDataApi : updateSchoolDataApi
    api(formData.value)
      .then((res) => {
...
//#region 删
const handleDelete = (row: SchoolData) => {
  ElMessageBox.confirm(`正在删除：${row.schoolName}，确认删除？`, "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(() => {
    delSchoolDataApi(row.schoolId).then(() => {
...
const handleSelectionChange = (val: SchoolData[]) => {
  let arr: number[] = [] 
  val.forEach((item) => {
...
//#region 改
const handleUpdate = (row: SchoolData) => {
  dialogVisible.value = true
  formData.value = cloneDeep(row)
  formData.value.id = "修改学院名称"
}
//#region 查
const tableData = ref<SchoolData[]>([])
const keyword = ref("")
const getTableData = () => {
  loading.value = true
  getSchoolPageDataApi({
...
//     (data) =>
//       !keyword.value ||
//       data.schoolId.toString().includes(keyword.value) ||
//       data.schoolName.toLowerCase().includes(keyword.value.toLowerCase())
//   )
// )
const resetSearch = () => {
  keyword.value = ""
...
const downloadTemplate = () => {
  window.open("http://localhost:3333/api/download?fileName=学院信息表")
}
...
<template>
  <div class="app-container">
    <el-card v-loading="loading" shadow="never" class="search-wrapper">
      <el-form :inline="true">
        <el-form-item prop="username" label="关键词">
          <el-input v-model="keyword" placeholder="请输入学院名称" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" @selection-change="handleSelectionChange" max-height="350">
          ...
          <el-table-column prop="schoolId" label="学院编号" sortable align="center" />
          <el-table-column prop="schoolName" label="学院名称" align="center" />
    ...
    <el-dialog v-model="uploadVisible" title="文件上传" width="30%" @closed="resetFile">
      <el-upload
        ref="uploadRef"
        action="http://localhost:3333/api/importSchool"
        class="upload-demo"
        drag
        accept=".xlsx, .xls"
        :file-list="fileList"
    ...
    <el-dialog
      ...
      :title="formData.id === undefined ? '新增学院' : '修改学院名称'"
      @closed="resetForm"
      width="30%"
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="left">
        <el-form-item prop="schoolId" label="学院编号" v-if="formData.id === '修改学院名称'">
          <el-input v-model="formData.schoolId" disabled/>
        </el-form-item>
        <el-form-item prop="schoolName" label="学院名称">
