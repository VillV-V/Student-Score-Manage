...
import { addTableDataApi, delTableDataApi, delBatchesDataApi, updateTableDataApi, getTableDataApi, exportFileApi } from "@/api/student"
import { type StudentData } from "@/api/student/types/student"
import { getStuClassDataApi } from "@/api/stu-class"
...
const formData = ref<StudentData>(cloneDeep(DEFAULT_FORM_DATA))
//校验手机号
...
const formRules: FormRules<StudentData> = {
  ...
  className: [{ required: true, trigger: "blur", message: "请选择所属班级" }],
...
const handleDelete = (row: StudentData) => {
  ElMessageBox.confirm(`正在删除学生：${row.studentName}，确认删除？`, "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(() => {
    delTableDataApi(row.studentId).then(() => {
...
const handleSelectionChange = (val: StudentData[]) => {
  const arr: number[] = []
  val.forEach((item) => {
...
//#region 改
const handleUpdate = (row: StudentData) => {
  getClassData()
  dialogVisible.value = true
  formData.value = cloneDeep(row)
  title.value = "修改学生信息"
  formData.value.id = "修改学生信息"
}
//修改密码
const UpdatePwd = (row: StudentData) => {
  isLock.value = true
  dialogVisible.value = true
  formData.value = cloneDeep(row)
  title.value = "修改学生密码"
  formData.value.id = "修改学生密码"
}

//获取班级列表
const classData = ref<StuClassData[]>([])
const getClassData = () => {
  getStuClassDataApi()
    .then(({ data }) => {
...
const filterGender = (value: string, row: StudentData) => {
...
const stuClassFilter = computed(
  () => {
    const classSet = new Set(tableData.value.map(item => item.className))
    return stuClass.value = Array.from(classSet).map(className => ({
      text: className,
      value: className,
    }));
  }
)
const filterStuClass = (value: string, row: StudentData) => {
...
const filterMajor = (value: string, row: StudentData) => {
...
const filterSchool = (value: string, row: StudentData) => {
  return row.schoolName === value;
}
...
const downloadTemplate = () => {
  window.open("http://localhost:3333/api/download?fileName=学生信息表")
}
  ...
  <div class="app-container">
    ...
    <el-card v-loading="loading" shadow="never">
      <div class="toolbar-wrapper">
        <div>
          <el-button type="primary" :icon="CirclePlus" @click="handleAdd">新增学生</el-button>
      ...
      <div class="table-wrapper">
        <el-table :data="tableData" @selection-change="handleSelectionChange" max-height="350px">
          ...
          <el-table-column prop="className" label="所属班级" align="center" :filters="stuClassFilter" :filter-method="filterStuClass" />
    ...
    <el-dialog v-model="uploadVisible" title="文件上传" width="30%" @closed="resetFile">
      <el-upload
        ref="uploadRef"
        action="http://localhost:3333/api/importStudent"
        class="upload-demo"
        drag
        accept=".xlsx, .xls"
        :file-list="fileList"
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="left">
        <el-form-item prop="studentId" label="学号" v-if="formData.id !== undefined">
          <el-input v-model="formData.studentId" disabled />
        </el-form-item>
        <el-form-item prop="studentName" label="姓名">
          <el-input v-model="formData.studentName" placeholder="请输入姓名" :disabled="isLock"/>
        </el-form-item>
        ...
        <el-form-item prop="className" label="所属班级">
          <el-select v-model="formData.classId" placeholder="请选择所属班级" filterable clearable @change="saveName" :disabled="isLock">
            <el-option v-for="item in classData" :key="item.id" :label="item.className" :value="item.classId" />
          </el-select>
        </el-form-item>
        <el-form-item prop="password" label="密码" v-if="formData.id != '修改学生信息'">
